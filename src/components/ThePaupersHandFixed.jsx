...
const ThePaupersHandFixed = ({ 
  ...
  // Debug logging for shop listings
  useEffect(() => {
    console.log('Shop listings received:', shopListings);
    if (Array.isArray(shopListings)) {
      const uniqueCats = [...new Set(shopListings.map(item => item.category).filter(Boolean))];
  ...
  // Get unique categories from shop listings with better error handling
  const categories = useMemo(() => {
    // Always start with 'All' category
    const uniqueCategories = ['All'];
    
    // Safely extract unique categories from shopListings
    if (Array.isArray(shopListings) && shopListings.length > 0) {
      // Use Set to automatically handle uniqueness
      const categorySet = new Set(shopListings
        .map(listing => listing.category)
        .filter(category => category && typeof category === 'string'));
      
      // Add all unique categories to our array
      uniqueCategories.push(...categorySet);
    }
    
    console.log('Generated category tabs:', uniqueCategories);
  ...
  const filteredShopListings = useMemo(() => {
    if (!Array.isArray(shopListings)) {
      console.error('shopListings is not an array:', shopListings);
      return [];
    }
      ...
      <div className="shop-content">
        ...
        <div className="shop-items-section">
          ...
          <div className="category-tabs">
            <FontAwesomeIcon icon={faTags} style={{ color: '#d4b16a', marginRight: '0.5rem' }} />
...
export default ThePaupersHandFixed; 